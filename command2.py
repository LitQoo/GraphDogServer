...
class CommandHandler(SessionBaseHandler):
	def post(self):
		...
		aID=ga[3]
		actions = [action]
		if type(param)==dict and type(param.get('subactions')) == list:
			actions+=param.get('subactions')

		if type(param)==dict and type(param.get('subactions'))==str:
			actions.append(param.get('subactions'))
		...
		#기본리턴값
		result = {'action':actions[0] , 'callback': param.get('callback'), 'timestamp':int(cTime),'createTime':str(cTime), 'version':version}
...
#					logging.info('insert to 7')
			...
			result['scorelist'] = result['scorelist'][:10]
			result['state']='ok'
		...
		if 'getmaxscores' in actions: ###############################################################################
			...
			for minfo in weeklylist:
				...
				result['maxscorelist'].append(resultdata)
				r+=1
		...
		if 'getweeklyscores' in actions: ################################################################################
			...
			for minfo in maxlist:
				...
				result['weeklyscorelist'].append(resultdata)
				r+=1
...
##					rank=rank+1
		...
		if 'getrequests' in actions: #######################################################################################
			...
			if type(auInfo.requests) == list:
				...
				result['requestlist']=copy.deepcopy(auInfo.requests)
